...
export function parseDuration(durationString) {
    ...
    switch (unit) {
        ...
        default: throw new TitanBotError(`Unknown duration unit: ${unit}`, ErrorTypes.VALIDATION, 'Please use s (seconds), m (minutes), h (hours), or d (days).', { unit });
    }
    const maxDuration = GIVEAWAY_CONFIG.maximumDuration ?? 30 * 24 * 60 * 60 * 1000;
    if (ms > maxDuration) throw new TitanBotError(`Duration exceeds maximum: ${ms}ms > ${maxDuration}ms`, ErrorTypes.VALIDATION, `Maximum duration is ${Math.floor(maxDuration / (24 * 60 * 60 * 1000))} days.`, { requestedMs: ms, maxMs: maxDuration });
    const minDuration = GIVEAWAY_CONFIG.minimumDuration ?? 10 * 1000;
    if (ms < minDuration) throw new TitanBotError(`Duration below minimum: ${ms}ms < ${minDuration}ms`, ErrorTypes.VALIDATION, `Minimum duration is ${Math.ceil(minDuration / 1000)} seconds.`, { requestedMs: ms, minMs: minDuration });
    return ms;
}

export function validatePrize(prize) {
    if (!prize || typeof prize !== 'string') throw new TitanBotError('Prize must be a non-empty string', ErrorTypes.VALIDATION, 'Please provide a valid prize description.', { prize });
    const trimmed = prize.trim();
    if (trimmed.length === 0 || trimmed.length > 256) throw new TitanBotError(`Prize length out of range: ${trimmed.length}`, ErrorTypes.VALIDATION, 'Prize must be between 1 and 256 characters.', { length: trimmed.length });
    return trimmed;
}
...
export function validateWinnerCount(winnerCount) {
    ...
    if (!Number.isInteger(winnerCount) || winnerCount < minimumWinners || winnerCount > maximumWinners) throw new TitanBotError(`Invalid winner count: ${winnerCount}`, ErrorTypes.VALIDATION, `Winner count must be between ${minimumWinners} and ${maximumWinners}.`, { winnerCount, minimumWinners, maximumWinners });
}
...
export function createGiveawayButtons(ended = false) {
    try {
        ...
        if (ended) {
            row.addComponents(
                new ButtonBuilder().setCustomId('giveaway_reroll').setLabel('🎲 Reroll').setStyle(ButtonStyle.Secondary).setDisabled(false),
                new ButtonBuilder().setCustomId('giveaway_view').setLabel('👁️ View Winners').setStyle(ButtonStyle.Primary).setDisabled(false)
            );
        } else {
            row.addComponents(
                new ButtonBuilder().setCustomId('giveaway_join').setLabel('🎉 Join').setStyle(ButtonStyle.Primary).setDisabled(false),
                new ButtonBuilder().setCustomId('giveaway_end').setLabel('🛑 End').setStyle(ButtonStyle.Danger).setDisabled(false)
            );
        }
        return row;
    } catch (error) {
        logger.error('Error creating giveaway buttons:', error);
        throw new TitanBotError('Failed to create giveaway buttons', ErrorTypes.UNKNOWN, 'An internal error occurred while creating interactive buttons.', { error: error.message });
    }
}
...
export function selectWinners(participants, winnerCount) {
    ...
    try {
        ...
        throw new TitanBotError('Failed to select winners', ErrorTypes.UNKNOWN, 'An error occurred while selecting winners.', { error: error.message, participantCount: participants.length });
    }
}
...
export async function endGiveaway(client, giveaway, guildId, endedBy) {
    try {
        if (!giveaway) throw new TitanBotError('Giveaway object is null or undefined', ErrorTypes.VALIDATION, 'Cannot end a non-existent giveaway.', { giveaway });
        if (giveaway.ended === true || giveaway.isEnded === true) throw new TitanBotError(`Giveaway ${giveaway.messageId} is already ended`, ErrorTypes.VALIDATION, 'This giveaway has already ended.', { giveawayId: giveaway.messageId, status: 'already_ended' });
        const participants = giveaway.participants || [];
        const winners = selectWinners(participants, giveaway.winnerCount || 1);
        const updatedGiveaway = { ...giveaway, ended: true, isEnded: true, winnerIds: winners, endedAt: new Date().toISOString(), endedBy: endedBy, participantCount: participants.length };
        logger.info(`Ending giveaway ${giveaway.messageId}: selected ${winners.length} winners from ${participants.length} entries`);
        return { giveaway: updatedGiveaway, winners, participantCount: participants.length };
    } catch (error) {
        ...
        throw new TitanBotError('Failed to end giveaway', ErrorTypes.UNKNOWN, 'An error occurred while ending the giveaway.', { error: error.message, giveawayId: giveaway?.messageId });
    }
}
...
export async function checkGiveaways(client) {
    try {
        ...
        // Giveaways are stored through the guild-key/value layer by saveGiveaway().
        // Reading the same source here avoids relying on the separate SQL giveaway
        // table, which can have a different ends_at value and caused delayed draws.
        ...
        for (const guild of client.guilds.cache.values()) {
            ...
            for (const giveaway of giveaways) {
                if (!giveaway?.messageId || giveaway.ended === true || giveaway.isEnded === true) continue;
                const endTime = Number(giveaway.endsAt ?? giveaway.endTime);
                if (!Number.isFinite(endTime) || now < endTime) continue;
                endedGiveaways.push({ guildId: guild.id, giveaway });
            }
        }
        ...
        for (const { guildId, giveaway } of endedGiveaways) {
            try {
                ...
                if (!channel?.isTextBased()) {
                    logger.debug(`Channel ${giveaway.channelId} not found for giveaway ${giveaway.messageId}`);
                    continue;
                }

                const message = await channel.messages.fetch(giveaway.messageId).catch(() => null);
                if (!message) {
                    logger.debug(`Message ${giveaway.messageId} not found for giveaway in channel ${giveaway.channelId}`);
                    continue;
                }
                ...
                logger.info(`Ended giveaway ${giveaway.messageId} in guild ${guildId}`);
            } catch (error) {
                logger.error(`Error processing giveaway ${giveaway?.messageId || 'unknown'}:`, error);
            }
        }
    } catch (error) {
